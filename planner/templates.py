...
def create_daily_page(pdf: FPDF, current_date: date) -> None:
    """
    Creates a daily page with header, tasks, and notes sections.
    """
    pdf.add_page()
    ...
    for _ in range(15):
        ...
        y = pdf.get_y()
        pdf.line(x_start, y + 2, x_start + 180, y + 2)
        pdf.ln(6)
...
def create_weekly_overview(pdf: FPDF, week_start_date: date) -> None:
    """
    Creates a weekly overview page with goals and a habit tracker.
    """
    pdf.add_page()
    pdf.set_font(*FONT_TITLE)
...
def create_monthly_title(pdf: FPDF, month_name: str) -> None:
    """
    Creates a monthly title page with space for key highlights.
    """
    pdf.add_page()
    pdf.set_font(*FONT_TITLE)
